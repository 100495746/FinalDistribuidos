...
#include <stdlib.h>
#include <string.h>
#include "usuario.h" // <-- Add this line if struct Usuario is defined in usuario.h
...
int registrar_usuario(const char *nombre) {
    ...
    // Reservar espacio para un nuevo usuario
    Usuario *nuevo = realloc(usrpt, (num_usuarios + 1) * sizeof(Usuario));
    if (nuevo == NULL) {
