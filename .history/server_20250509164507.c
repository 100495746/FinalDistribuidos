...
void *handle_client(void *arg) {
    ...
    int retFlag;
void * retVal = register(comando, cliente_sd, retFlag);
if (retFlag == 1) return retVal;
    ...
    int retFlag;
    void *retVal = publish(comando, cliente_sd, retFlag);
    if (retFlag == 1)
        return retVal;
...
void * register(char  comando[256], int cliente_sd, int &retFlag)
{
retFlag = 1;
...
void *publish(char comando[256], int cliente_sd, int &retFlag)
{
    retFlag = 1;
    if (strcmp(comando, "PUBLISH") == 0)
    {
        ...
        u->ficheros = nuevo;
        // "construimos" el struct del nuevo file (path y descripcion)
