...
int registrar_usuario(const char *nombre) {
    ...
    if (num_usuarios >= capacidad_usuarios) {
        int nueva_capacidad = capacidad_usuarios == 0 ? 10 : capacidad_usuarios * 2;
        Usuario *nuevo = malloc(nueva_capacidad * sizeof(Usuario));
        if (nuevo == NULL) {
        ...
        usrpt = nuevo;
        capacidad_usuarios = nueva_capacidad;
    }
