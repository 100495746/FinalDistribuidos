...
int registrar_usuario(const char *nombre) {
    ...
    strncpy(usrpt[num_usuarios].nombre, nombre, sizeof(usuarios[num_usuarios].nombre) - 1);
    usrpt[num_usuarios].nombre[sizeof(usrpt[num_usuarios].nombre) - 1] = '\0'; //aseguro terminación
    usrpt[num_usuarios].conectado = 0; // no conectado al ppio
    usrpt[num_usuarios].puerto = -1; //puerto inexistente al empezar ( no conectado aun)
    usrpt[num_usuarios].ip[0] = '\0'; // ip vacío hasta que se conecte
...
Usuario *buscar_usuario(const char *nombre){
    for (int i = 0; i < num_usuarios; i++) {
        if (strcmp(usrpt[i].nombre, nombre) == 0) {
            return &usrpt[i]; //devuelve lo encontrado
        }
    }
    return NULL;
}
