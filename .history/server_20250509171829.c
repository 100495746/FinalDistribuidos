...
void *connect_user(int cliente_sd){
    ...
    if (read(cliente_sd, puerto, sizeof(int)) < 1) {
        perror("Error leyendo el puerto del usuario");
        return NULL;
    }
    if (read(cliente_sd, conectado, sizeof(int)) < 1) {
        perror("Error leyendo el estado del usuario");
        return NULL;
    }
    if(conectado==0){
        perror("Usuario no conectado");
        return NULL;
    }
    Usuario *u = buscar_usuario(nombre);
    if (u == NULL) {
        resultado = -1;
        write(cliente_sd, &resultado, sizeof(int));
        return NULL;
    }

    //el cliente no tiene por que mandar su ip...

    struct sockaddr_in addr;
    //obtenemos la IP y puerto del cliente que se ha conectado
    //getpeername obtiene el address del cliente conectado al socket
    getpeername(cliente_sd, (struct sockaddr *)&addr, sizeof(struct sockaddr_in));

    // convertimos addr.sinaddr de binario a una cadena legible IPV6
    // AF_INET nos dice que es ipv4, y lo mete en u->ip
