...
void *handle_client(void *arg) {
    ...
    if (strcmp(comando, "PUBLISH") == 0) {
        ...
        if (read(cliente_sd, ip, sizeof(ip)) < 1) {
            perror("Error leyendo el estado de conexión del usuario");
            close(cliente_sd);
            return NULL;
        }
        ...
        if (u!= NULL){
            File *nuevo_array = malloc(u->ficheros)
            FILE *

        }

        Usuario *nuevo = malloc((u->num_ficheros+1) * sizeof(File));
        if (nuevo == NULL) {
            return 2; // Error de memoria
        }
        // Copiar los files existentes al nuevo array
        if (u != NULL) {
            memcpy(nuevo_array, u, u->num_ficheros * sizeof(Usuario));
            free(usrpt);
        }
        usrpt = nuevo;
